...
describe('Shared Knowledge routes', () => {
  ...
  // THE GRAPH TESTS MOVED WITH THE QUERIES (#300).
  //
  // Five cases lived here — totals from COUNT(*) rather than the page, every
  // list bounded, COUNT(DISTINCT) for the grouped agent list, truncated=false
  // when the page IS the corpus, and no edge to a collection that was cut. They
  // asserted the behaviour of four statements this route ran against its own
  // pool, and those statements queried tables that live in the KNOWLEDGE
  // service's database and do not exist in this one — so the route answered 500
  // on every call while these tests passed against a mock.
  //
  // Each has an equivalent in services/knowledge/tests/test_shared_graph.py,
  // next to the SQL it describes. What remains here is the only thing this
  // service is now responsible for: asking the other one. That is in
  // shared-knowledge-graph-proxied.test.ts.
