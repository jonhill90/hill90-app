...
# `auth` is deliberately ABSENT since 2026-08-01. The tenant path authenticates
# against HILL90'S Keycloak, realm `platform` — the realm production uses — so it
# no longer starts app-keycloak. Before this, a local login exercised the app's
# own realm: it proved the realm DESIGN and nothing about the tenancy.
#
# deploy/compose/overrides/local.auth.yml is now UNUSED — nothing drives it, since
# --standalone runs compose/local.yml's own Keycloak, not this override. It is kept
# rather than deleted, with a banner, for the same reason the db and auth compose
# files were kept after those retirements: a documented, restorable path beats a
# deletion nobody can reverse without archaeology.
STACKS="db api ai knowledge mcp minio ui"
...
# The app's own Keycloak hostname. Used ONLY by --standalone, which still runs
# the fork's Keycloak. The tenant path no longer starts app-keycloak at all.
APP_AUTH_HOST=app-auth

# Hill90's Keycloak. Since 2026-08-01 this is what the TENANT path authenticates
# against — realm \`platform\`, the same realm production uses.
AUTH_HOST=auth
...
# ---------------------------------------------------------------------------
# Tenant parity against Hill90's Keycloak
# ---------------------------------------------------------------------------

kc_platform() { docker exec "${KC_PLATFORM:-$(ev CONTAINER_PREFIX)keycloak}" /opt/keycloak/bin/kcadm.sh "$@"; }

kc_platform_login() {
  kc_platform config credentials --server http://localhost:8080 --realm master \
    --user "$(ev KC_ADMIN_USERNAME)" --password "$(ev KC_ADMIN_PASSWORD)" >/dev/null 2>&1
}

kc_ui_uuid() {
  kc_platform get clients -r "$(kc_realm)" -q clientId=hill90-ui --fields id 2>/dev/null \
    | python3 -c 'import sys,json; d=json.load(sys.stdin); print(d[0]["id"] if d else "")' 2>/dev/null
}

kc_realm() { local r; r=$(ev KC_REALM); printf '%s' "${r:-platform}"; }

# hill90-ui's secret is generated by HILL90's local.sh, so a literal here would be
# wrong the first time Hill90 regenerates it. Read it from the running realm.
ensure_platform_client_secret() {
  local uuid secret
  kc_platform_login || { echo "  ! Could not authenticate to the platform Keycloak — leaving AUTH_KEYCLOAK_SECRET as-is"; return 0; }
  uuid=$(kc_ui_uuid)
  [ -n "$uuid" ] || { echo "  ! hill90-ui is not in realm $(kc_realm) — run Hill90's scripts/keycloak.sh apply"; return 0; }
  secret=$(kc_platform get "clients/${uuid}/client-secret" -r "$(kc_realm)" --fields value 2>/dev/null \
           | python3 -c 'import sys,json; print(json.load(sys.stdin).get("value",""))' 2>/dev/null)
  [ -n "$secret" ] || { echo "  ! hill90-ui has no secret on the platform Keycloak"; return 0; }
  python3 - "$ENV_FILE" "$secret" <<'PYEOF'
import sys
p, sec = sys.argv[1], sys.argv[2]
lines = open(p).readlines()
if any(l.startswith("AUTH_KEYCLOAK_SECRET=") for l in lines):
    lines = [f"AUTH_KEYCLOAK_SECRET={sec}\n" if l.startswith("AUTH_KEYCLOAK_SECRET=") else l for l in lines]
else:
    lines.append(f"AUTH_KEYCLOAK_SECRET={sec}\n")
open(p, "w").writelines(lines)
PYEOF
  echo "  ✓ AUTH_KEYCLOAK_SECRET synced from realm $(kc_realm) (value not shown)"
}

# EVERY seeded account gets email, firstName AND lastName. Without all three the
# realm's default Verify Profile action diverts the login to
# required-action?execution=VERIFY_PROFILE, which looks EXACTLY like rejected
# credentials — same form, no error naming the profile. That cost this estate a day.
seed_platform_users() {
  local realm uuid spec u e f l role
  realm=$(kc_realm)
  kc_platform_login || { echo "  ! Could not authenticate to the platform Keycloak — skipping user seeding"; return 0; }
  uuid=$(kc_ui_uuid)
  [ -n "$uuid" ] || { echo "  ! hill90-ui missing — skipping user seeding"; return 0; }
  for spec in \
      "dev:dev@localhost:Dev:Local:admin" \
      "jon:jon@localhost:Jon:Hill:admin" \
      "testuser01:testuser01@localhost:Test:User01:user"; do
    IFS=: read -r u e f l role <<<"$spec"
    kc_platform create users -r "$realm" \
      -s "username=$u" -s enabled=true -s "email=$e" -s emailVerified=true \
      -s "firstName=$f" -s "lastName=$l" >/dev/null 2>&1 || true
    kc_platform set-password -r "$realm" --username "$u" --new-password "$u" >/dev/null 2>&1 || true
    kc_platform add-roles -r "$realm" --uusername "$u" --cclientid hill90-ui --rolename "$role" >/dev/null 2>&1 || true
  done
...
cmd_up() {
  ...
  # ONE-TIME MIGRATION. `auth` left STACKS on 2026-08-01, and compose only stops
  # what it manages — so an app-keycloak from before the change keeps running,
  # keeps its `keycloak` alias on the shared networks, and reintroduces exactly
  # the ambiguity this change removes. Remove it explicitly.
  if [ "$INFRA" = "1" ]; then
    local stale; stale="$(ev CONTAINER_PREFIX)app-keycloak"
    if docker inspect "$stale" >/dev/null 2>&1; then
      echo "  ! removing $stale — the tenant path uses Hill90's Keycloak now"
  ...
  mkdir -p "$ROOT/.local/agentbox-configs"
  gen_keys
  # BEFORE compose reads .env.local: the UI needs hill90-ui's secret, and that
  # secret is Hill90's to generate, not ours.
  [ "$INFRA" = "1" ] && ensure_platform_client_secret
...
EOF
  ...
  Through Traefik (Hill90 infra):
    ...
    Keycloak      http://$(ev AUTH_HOST).$dom:$port/realms/platform   (HILL90's — the tenancy)
    MCP gateway   http://$(ev AI_HOST).$dom:$port/mcp
    MinIO console http://$(ev STORAGE_HOST).$dom:$port
EOF
  cat <<EOF

  Log in to the UI as  dev / dev   (realm 'platform', on HILL90's Keycloak)
