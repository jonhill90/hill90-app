 ...
 * This suite creates a real workflow.
...
test.describe("Workflows Page", () => {
  ...
  test("renders workflows page with create button", async ({ page }) => {
    await login(page);
    await page.goto("/harness/workflows");
    await page.waitForLoadState("networkidle");

    await expect(page.getByText("Workflows")).toBeVisible({ timeout: 10_000 });
  ...
  test("can open create form", async ({ page }) => {
    ...
    await expect(page.getByText("Schedule (cron)")).toBeVisible({ timeout: 5_000 });
    await expect(page.getByPlaceholder("Daily Health Check")).toBeVisible();
  });
});
